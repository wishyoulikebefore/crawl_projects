...
today = judge_date()
fileName = "D:/python_stock/股票概念/地域概念/"+today
...
output = open("地域分类概念.txt","a")
targetDict = {}
collection = stockConcept_mongo()

def startCrawl_industryConcept():
    soup = getHtmlPage_GBK(url="http://q.10jqka.com.cn/dy/")
...
def crawl_stock(browser,conceptName):
    ...
    for item in locate:
        ...
        collection.pushConcept(stockCode,stockName,"regionConcept",conceptName)
        output.write("%s\t%s\n" %(stockName,stockCode))
